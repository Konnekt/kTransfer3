...
:: This is if you have Microsoft Platform SDK for Windows Server 2003 R2.
if exist "%ProgramFiles%\Microsoft Platform SDK for Windows Server 2003 R2\SetEnv.cmd" (
...
if defined SMARTWIN_TMP (
	...
	if exist %SMARTWIN_TMP%\SetEnv.Bat (
		call %SMARTWIN_TMP%\SetEnv.Bat
		set SMARTWINok_b=true
		goto STARTCOMPILING
	)
	if exist %SMARTWIN_TMP%\SetEnv.Cmd (
		call %SMARTWIN_TMP%\SetEnv.Cmd
		set SMARTWINok_b=true
		goto STARTCOMPILING
	)
)
